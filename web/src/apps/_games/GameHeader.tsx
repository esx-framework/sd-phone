...
export function GameHeader({ title, accent = '#fff', onBack, right }: GameHeaderProps) {
    return (
        <div className="relative flex shrink-0 items-center justify-center px-4 pb-1 pt-1">
            <button type="button" onClick={onBack} aria-label={t('games.back', 'Back')} className="absolute start-3 flex items-center active:opacity-60" style={{ color: accent }}>
            ...
            {right && <div className="absolute end-3 flex items-center">{right}</div>}
        </div>
    );
}
